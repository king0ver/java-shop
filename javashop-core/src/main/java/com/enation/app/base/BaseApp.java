...
public class BaseApp extends App {
	...
	protected void cleanCache() {
		...
		// 清除挂件缓存
		cache.remove("widget_" + userid + "_" + siteid);

	 
		// 清除themuri缓存
		cache.remove(ThemeUriCacheProxy.LIST_KEY_PREFIX + userid + "_" + siteid);
		
		
		// 清除SiteMenu缓存
		cache.remove(SiteMenuCacheProxy.MENU_LIST_CACHE_KEY + "_" + userid + "_"+ siteid);
