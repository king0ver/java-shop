...
public class UserAdminController extends GridController {
	...
	public ModelAndView passEdit() throws Exception {
		ModelAndView view = new ModelAndView();
		HttpSession session = ThreadContextHolder.getSession();
		AdminUser user = null;
		if (session != null) {
			user = (AdminUser) session.getAttribute(UserConext.CURRENT_ADMINUSER_KEY);
		}
		view.addObject("adminUser",user);
		view.setViewName("/adminthemes/version3/modify_password");
		return view;
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="/pass-edit-save")
	public JsonResult passEditSave(AdminUser adminUser) throws Exception {
		
		//判断是否为演示站点
		AdminUser user  = UserConext.getCurrentAdminUser();
		if(EopSetting.IS_DEMO_SITE && user!= null && user.getFounder() != 1){
			return JsonResultUtil.getErrorJson("抱歉，当前为演示站点，所以不能修改这些示例数据，请下载安装包在本地体验这些功能！");
		}
		...
		try {
			...
			if(flag){
				...
				this.adminUserManager.edit(adminUser);
				return JsonResultUtil.getSuccessJson("管理员账号密码修改成功");
			}
		} catch (RuntimeException e) {
			e.printStackTrace();
			logger.error(e,e.fillInStackTrace());
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="/old-password")
	public JsonResult oldPassword(String username,String password) throws Exception {
		
		List<AdminUser> users = adminUserManager.getByUsername(username);
		for(AdminUser user : users) {
			if(!user.getPassword().equals(StringUtil.md5(password))) {
				return JsonResultUtil.getErrorJson("输入原密码错误，请重新输入！！");
			}
		}
		return JsonResultUtil.getSuccessJson("原密码正确！！");
	}
