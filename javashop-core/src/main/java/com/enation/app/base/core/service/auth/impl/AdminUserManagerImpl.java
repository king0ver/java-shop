...
public class AdminUserManagerImpl  implements	IAdminUserManager {
	...
	public int loginBySys(String username, String password) {
		...
		if (userList == null || userList.size() == 0) {
			throw new UnProccessableServiceException(ErrorCode.INVALID_REQUEST_PARAMETER, "此用户不存在！");
		}
		AdminUser user = userList.get(0);

		if (!password.equals(user.getPassword())) {
			throw new UnProccessableServiceException(ErrorCode.INVALID_REQUEST_PARAMETER, "密码错误！");
		}

		if (user.getState() == 0) {
			throw new UnProccessableServiceException(ErrorCode.INVALID_REQUEST_PARAMETER, "此用户已经被禁用！");
		}
	...
	@Override
	public List<AdminUser> getByUsername(String username) {
		String sql = "select * from es_adminuser where username=?";
		List<AdminUser> userList = this.daoSupport.queryForList(sql, AdminUser.class, username);
		return userList;
	}
