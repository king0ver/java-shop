...
public class UploadImageApiController {
		...
		try {
			if (file != null) {
				if (!FileUtil.isAllowUpImg(file.getOriginalFilename())) {
					return JsonResultUtil.getErrorJson("不允许上传的文件格式，请上传gif,jpg格式文件。");
				} else {
					InputStream stream = null;
					stream = file.getInputStream();
					IUploader uploader=uploadFactory.getUploader();
					String fsImgPath = uploader.upload(file);
					// return
	...
	public String uploadImg(MultipartFile upfile) {
		...
		IUploader uploader=uploadFactory.getUploader();
		/** 图片上传 */
		String path = uploader.upload(upfile);
		...
		try {
			if (image != null) {
				if (!FileUtil.isAllowUpImg(image.getOriginalFilename())) {
					return JsonResultUtil.getErrorJson("不允许上传的文件格式，请上传jpg，png，gif格式文件。");
				} else {
					/** 获取文件操作方案 */
					IUploader uploader=uploadFactory.getUploader();
					/** 图片上传 */
					String fsImgPath = uploader.upload(image);
					return "{\"img\":\"" + fsImgPath + "\",\"fsimg\":\"" + fsImgPath
							+ "\"}";
				}
	...
	public Object uploadBase64(int base64Len, String base64,String type) {
		...
		// 如果图片上传成功
		if (Base64ToPicture.GenerateImage(base64, path)) {
			Map<String, Object> map = new HashMap<String, Object>();
