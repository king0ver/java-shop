...
public class AlipayRefundExcutor extends AlipayPluginConfig{
	...
	public RefundPartVo queryRefundStatus(RefundPartVo refund) {
		try {
			...
			//商户订单号，商户网站订单系统中唯一订单号
			String out_trade_no = refund.getTrade_sn();
			//请求退款接口时，传入的退款请求号，如果在退款请求时未传入，则该值为创建交易时的外部交易号，必填
			String out_request_no = "HZ" + refund.getRefund_sn();
			...
			if(response.isSuccess()){
				...
				if(res.get("out_trade_no")!=null){
					refund.setRefund_status(RefundStatus.completed.value());
				}else{
					refund.setRefund_status(RefundStatus.refundfail.value());
				}
		    } 
