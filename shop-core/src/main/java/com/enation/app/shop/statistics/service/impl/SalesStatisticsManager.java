...
import com.enation.app.shop.statistics.service.ISalesStatisticsManager;
...
//			this.daoSupport.execute(createfunction);
	...
	public List<DayAmount> statisticsDay_Amount() {
		...
		String sql = "";
		if(EopSetting.DBTYPE.equals("1")){//是mysql
			sql = "select sum(a.order_amount) as amount, Date_format(FROM_UNIXTIME(a.create_time / 1000),'%Y-%m-%d') as mo from es_order a where a.order_status = '" + OrderStatus.COMPLETE + "' and Date_format(FROM_UNIXTIME(a.create_time / 1000),'%Y-%m') = ?  group by mo";
		}else if(EopSetting.DBTYPE.equals("2")){
			sql = "select sum(a.order_amount) as amount, to_char(TO_DATE('01011970','mmddyyyy')+1/24/60/60*(a.create_time / 1000),'yyyy-mm-dd') as mo from es_order a where a.order_status = '" + OrderStatus.COMPLETE + "' and to_char(TO_DATE('01011970','mmddyyyy')+1/24/60/60*(a.create_time / 1000),'yyyy-mm') = ?  group by to_char(TO_DATE('01011970','mmddyyyy')+1/24/60/60*(a.create_time / 1000),'yyyy-mm-dd')";
		}else{
			sql = "select sum(a.order_amount) as amount, substring(convert(varchar(10),dateadd(ss,create_time/1000 + 28800,'1970-01-01'),120),1,10) as mo from es_order a where a.order_status = '" + OrderStatus.COMPLETE + "' and substring(convert(varchar(10),dateadd(ss,create_time/1000 + 28800,'1970-01-01'),120),1,7) = ?  group by substring(convert(varchar(10),dateadd(ss,create_time/1000 + 28800,'1970-01-01'),120),1,10)";
		}
		List<Map> list = this.daoSupport.queryForList(sql, year);
	...
	public List<DayAmount> statisticsDay_Amount(String monthinput) {
		...
		if("1".equals(EopSetting.DBTYPE)){//是mysql
			...
			sql = "select sum(order_amount) as amount, substring(convert(varchar(10),dateadd(ss,create_time/1000 + 28800,'1970-01-01'),120),1,10) as mo from es_order a where a.order_status = '" + OrderStatus.COMPLETE + "' and substring(convert(varchar(10),dateadd(ss,create_time/1000 + 28800,'1970-01-01'),120),1,7) = ? group by substring(convert(varchar(10),dateadd(ss,create_time/1000 + 28800,'1970-01-01'),120),1,10)";
		}
