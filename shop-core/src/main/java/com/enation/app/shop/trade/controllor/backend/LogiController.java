...
public class LogiController extends GridController {
	...
	public JsonResult saveAdd(Logi log){
		try {
			Logi logi = new Logi();
			logi.setCode(log.getCode());
			logi.setCode(log.getKdcode());
			logi.setName(log.getName());
			Logi logicode = logiManager.getLogiByCode(log.getCode());
			Logi logikdcode = logiManager.getLogiBykdCode(log.getKdcode());
			Logi loginame = logiManager.getLogiByName(log.getName());
			if(loginame != null){
				return JsonResultUtil.getErrorJson("快递公司名称不能相同");
			}
			if(logikdcode != null){
				return JsonResultUtil.getErrorJson("快递鸟快递公司代码不能相同");
			}
			if(logicode != null){
				return JsonResultUtil.getErrorJson("快递公司代码不能相同");
			}else{
				logiManager.saveAdd(log);
	...
	public JsonResult saveEdit(Logi log){
		try {
			Logi logi = new Logi();
			logi.setId(log.getId());
			logi.setCode(log.getKdcode());
			logi.setCode(log.getCode());
			logi.setName(log.getName());
			Logi logicode = logiManager.getLogiByCode(log.getCode());
			Logi logikdcode = logiManager.getLogiBykdCode(log.getKdcode());
			Logi loginame = logiManager.getLogiByName(log.getName());
			if(logikdcode != null && logikdcode.getId() != log.getId()){
				return JsonResultUtil.getErrorJson("快递鸟快递公司代码不能相同");
			}
			if(loginame != null && loginame.getId() != log.getId()){
				return JsonResultUtil.getErrorJson("快递公司名称不能相同");
			}
			if(logicode != null && logicode.getId() != log.getId()){
				return JsonResultUtil.getErrorJson("快递公司代码不能相同");
			}else{
				logiManager.saveEdit(log);
