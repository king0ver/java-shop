...
import com.enation.app.shop.goods.service.IGoodsTagManager;
...
public class GoodsTagManager implements IGoodsTagManager {
	 ...
	 * 
	 * @see
	 * com.enation.app.shop.core.goods.service.IGoodsTagManager#getGoodsList(int,
	 * int, int)
	 */
	@Override
	public Page getGoodsList(int tagid, int page, int pageSize) {
		...
		sql.append(
				"select g.goods_id,g.goods_name,g.price,g.thumbnail,r.tag_id,r.ordernum from es_tag_rel  r LEFT JOIN es_goods g ON g.goods_id=r.rel_id where g.disabled=0 and g.market_enable=1");
	...
	public Page getGoodsList(int tagid, int catid, int page, int pageSize) {
		...
		StringBuffer sql = new StringBuffer();
		sql.append(
				"select g.goods_id,g.name,r.tag_id,r.ordernum from es_tag_rel r LEFT JOIN es_goods g ON g.goods_id=r.rel_id where g.disabled=0 and g.market_enable=1");
		sql.append(" and r.tag_id = ?");

		sql.append(" and  g.cat_id in(");
		sql.append("select c.cat_id from es_goods_category");
		sql.append(" c where c.cat_path like '" + cat.getCategory_path() + "%')");
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.enation.app.shop.core.goods.service.IGoodsTagManager#addTag(int,
	 * int)
	 */
	@Override
	@Log(type = LogType.GOODS, detail = "批量添加标签")
	public void addTag(int tagId, int goodsId) {
		if (this.daoSupport.queryForInt("SELECT COUNT(0) FROM es_tag_rel WHERE tag_id=? AND rel_id=?", tagId,
				goodsId) <= 0) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.enation.app.shop.core.goods.service.IGoodsTagManager#removeTag(int,
	 * int)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * com.enation.app.shop.core.goods.service.IGoodsTagManager#updateOrderNum(java.
		...
		if (goodsIds != null && goodsIds.length > 0) {
			for (int i = 0; i < goodsIds.length; i++) {
				if (goodsIds[i] != null && tagids[i] != null && ordernum[i] != null) {
					try {
						this.daoSupport.execute("UPDATE es_tag_rel set ordernum=? WHERE tag_id=? AND rel_id=?",
	...
	public List getGoodsList(int tagid) {
		StringBuffer sql = new StringBuffer();
		sql.append(
				"select g.goods_id,g.name,r.tag_id,r.ordernum,g.thumbnail,g.price from es_tag_rel r LEFT JOIN es_goods g ON g.goods_id=r.rel_id where g.disabled=0 and g.market_enable=1");
		sql.append(" and r.tag_id = ?");
		sql.append(" order by r.ordernum desc");
