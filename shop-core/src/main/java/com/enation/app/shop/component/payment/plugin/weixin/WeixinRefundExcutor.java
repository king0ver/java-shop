...
public class WeixinRefundExcutor extends WeixinPuginConfig{
	...
	public boolean returnPay(RefundBill bill) {
		...
		params.put("total_fee", toFen(0.01d));//交易金额
	...
	public RefundPartVo queryRefundStatus(RefundPartVo refund) {
		...
		params.put("nonce_str", StringUtil.getRandStr(10));
		/** 商户系统内部订单号 */
		params.put("out_refund_no",refund.getRefund_sn());
		/** 签名	 */
		String sign = WeixinUtil.createSign(params, key);
		params.put("sign", sign);
		try {
...
//				CHANGE—退款异常，退款到银行发现用户的卡作废或者冻结了，导致原路退款银行卡失败，可前往商户平台（pay.weixin.qq.com）-交易中心，手动处理此笔退款。$n为下标，从0开始编号。
				if("SUCCESS".equals(status)){
					refund.setRefund_status(RefundStatus.completed.value());//完成
				}else if("PROCESSING".equals(status)){
					refund.setRefund_status(RefundStatus.refunding.value());//退款中
				}else {
					refund.setRefund_status(RefundStatus.refundfail.value());//退款失败
				}
