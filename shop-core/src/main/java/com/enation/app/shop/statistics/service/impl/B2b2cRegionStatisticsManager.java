...
@Service
public class B2b2cRegionStatisticsManager implements IB2b2cRegionStatisticsManager {
	 ...
	 */
	@Override
	public String getRegionStatistics(Integer type,Integer cycle_type,Integer year,Integer month,String store_id) {
		// 新建地区计划
		...
		switch (type) {
			case 1:
				list = getRegionOrderMemberNum(cycle_type,year,month,store_id);
				break;
			case 2:
				list = this.getRegionOrderNum(cycle_type,year,month,store_id);
				break;
			default:
				list = this.getRegionOrderPrice(cycle_type,year,month,store_id);
				break;
		}
	 ...
	 */
	@Override
	public List regionStatisticsList(Integer type, String sort,Integer cycle_type,Integer year,Integer month,String store_id) {
		...
		sql.append("WHERE disabled!=1 and member_id IN(SELECT o.member_id from es_order o WHERE o.ship_provinceid=s.region_id AND o.order_status= '"+ OrderStatus.COMPLETE + "'");
		...
		sql.append(",(SELECT COUNT(o.order_id) from es_order o WHERE o.ship_provinceid=s.region_id AND o.order_status= '"+ OrderStatus.COMPLETE + "'");
		...
		sql.append(",(SELECT SUM(o.paymoney) from es_order o WHERE o.ship_provinceid=s.region_id AND o.order_status= '"+ OrderStatus.COMPLETE + "'");
	 ...
	 */
	private List getRegionOrderMemberNum(Integer cycle_type,Integer year,Integer month, String store_id) {
		// 拼装 区域会员下单数量SQL
		StringBuffer sql = new StringBuffer("select s.local_name,(SELECT COUNT(member_id) from es_member ");
		sql.append("WHERE disabled!=1 and member_id IN(SELECT o.member_id FROM es_order o WHERE o.ship_provinceid=s.region_id AND o.order_status='"+ OrderStatus.COMPLETE+"'");
	 ...
	 * @return 区域下单数量 JSON
	 */
	private List getRegionOrderNum(Integer cycle_type,Integer year,Integer month, String store_id) {
		StringBuffer sql = new StringBuffer("select s.local_name,(SELECT COUNT(o.order_id) FROM es_order o WHERE o.ship_provinceid=s.region_id AND o.order_status= '"+ OrderStatus.COMPLETE + "'");
	 ...
	 * @return 区域下单金额 JSON
	 */
	private List getRegionOrderPrice(Integer cycle_type,Integer year,Integer month, String store_id) {
		StringBuffer sql = new StringBuffer("select s.local_name,(SELECT SUM(o.paymoney) from es_order o WHERE o.ship_provinceid=s.region_id AND o.order_status= '"+ OrderStatus.COMPLETE + "'");
		//判断周期模式 1为月
		if(cycle_type==1){
			sql.append(" AND o.complete_time<"+getMaxvalType1(year,month)+" AND o.complete_time > "+getMinvalType1(year, month));
		}else{
