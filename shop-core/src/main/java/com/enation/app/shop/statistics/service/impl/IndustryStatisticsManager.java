...
import com.enation.app.shop.statistics.service.IIndustryStatisticsManager;
...
public class IndustryStatisticsManager  implements
		...
		for (int i = 0; i < treeRoot.size(); i++) {
			...
			if (type == 1) {
				...
				String temSql = "select SUM(oi.price) sump from es_order_items oi left join es_order od on oi.order_sn = od.sn where  oi.cat_id in (select category_id from es_goods_category gc where gc.category_path like '0|"
						+ treeRoot.get(i).get("cat_id") + "%') and od.create_time BETWEEN "+this.getMinvalType1(year, month)+" and "+this.getMaxvalType1(year, month) +" and od.disabled = 0 AND ((od.payment_id = 3  AND od.order_status = '"
				...
				// 年份统计
				String temSql = "select SUM(oi.price) sump from es_order_items oi left join es_order od on oi.order_sn = od.sn where  oi.cat_id in (select category_id from es_goods_category gc where gc.category_path like '0|"
						+ treeRoot.get(i).get("cat_id") + "%') and od.create_time BETWEEN "+this.getMinvalType0(year)+" and "+this.getMaxvalType0(year)+" and od.disabled = 0 AND ((od.payment_id = 3  AND od.order_status = '"
				+ OrderStatus.COMPLETE
				+ "') OR ( od.pay_status = '"
				+ PayStatus.PAY_YES
				+ "'))"; 
		...
		for (int i = 0; i < treeRoot.size(); i++) {
			...
			if (type == 1) {
				...
				String temSql = "select count(od.order_id) sump from  es_order od left join es_order_items oi on oi.order_sn = od.sn where  oi.cat_id in (select category_id from es_goods_category gc where gc.category_path like '0|"
						+ treeRoot.get(i).get("category_id") + "%') and od.create_time BETWEEN "+this.getMinvalType1(year, month)+" and "+this.getMaxvalType1(year, month) +" and od.disabled = 0 AND ((od.payment_id = 3  AND od.order_status = '"
								+ OrderStatus.COMPLETE
								+ "')	OR ( od.pay_status = '"
								+ PayStatus.PAY_YES
								+ "'))"; 
				...
				// 年份统计
				String temSql = "select count(od.order_id) sump from es_order od left join es_order_items oi on oi.order_sn = od.sn where  oi.cat_id in (select category_id from es_goods_category gc where gc.category_path like '0|"
						+ treeRoot.get(i).get("category_id") + "%') and od.create_time BETWEEN "+this.getMinvalType0(year)+" and "+this.getMaxvalType0(year)+" and od.disabled = 0 AND ((od.payment_id = 3  AND od.order_status = '"
								+ OrderStatus.COMPLETE
								+ "')	OR ( od.pay_status = '"
								+ PayStatus.PAY_YES
								+ "'))"; 
	 ...
	 */
	@Override
	public List<Collect> listCollect(int cat_id,List<Category> cats) { 
		...
		for (Category cat : cats) {
			...
			//平均价格
			String avgSql = "select AVG(gs.price) as avg from es_goods gs where category_id in (select category_id from es_goods_category gc where gc.category_path like '%|"	+ cat.getCategory_id() + "|%') ";
			try {
			...
			String salesGoodsSql = "select count(0)from (select oi.goods_id from es_order_items oi " +
					"left join es_order od on oi.order_id = od.order_id where oi.category_id in " +
					...
					+" and od.disabled = 0 AND ((od.payment_id = 3  AND od.order_status = '"
					+ OrderStatus.COMPLETE
					+ "')	OR ( od.pay_status = '"
					+ PayStatus.PAY_YES
			...
			//无销量商品数 / 总商品数
			String residueSql = "select count(0) from es_goods g where g.category_id in (select category_id from es_goods_category gc where gc.category_path like '%|"	+ cat.getCategory_id() + "|%')";
			try {
			...
			//销售总额
			String countSql = "select sum(oi.price) as zonge,count(0) as zongs from es_order_items oi left join es_order od on oi.order_sn = od.sn where  oi.category_id in (select category_id from es_goods_category gc where gc.category_path like '%|"
					+ cat.getCategory_id() + "|%') and od.create_time BETWEEN "+startDate+" and "+endDate +" and od.disabled = 0 AND ((od.payment_id = 3  AND od.order_status = '"
							+ OrderStatus.COMPLETE
							+ "')	OR ( od.pay_status = '"
							+ PayStatus.PAY_YES
							+ "'))";
