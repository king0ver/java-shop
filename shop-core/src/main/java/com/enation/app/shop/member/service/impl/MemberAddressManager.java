...
public class MemberAddressManager implements IMemberAddressManager {
	...
	/*
	 * (non-Javadoc)
	 * @see com.enation.app.shop.core.member.service.IMemberAddressManager#get(java.lang.Integer)
	 */
	@Override
	public Address get(Integer addressid) {
		
		MemberAddress address = this.daoSupport.queryForObject("select * from es_member_address where addr_id = ?",MemberAddress.class, addressid);
		Address addr = new Address();
		if(address!=null){
			addr.setAddr_id(addressid);
			...
			addr.setProvince_id(address.getProvince_id());
			addr.setCity_id(address.getCity_id());
			addr.setRegion_id(address.getRegion_id());
			addr.setTown_id(address.getTown_id());
			addr.setMobile(address.getMobile());
			addr.setTel(address.getTel());
	...
	public Page getMemberAddress(int pageNo, int pageSize, int member_id) {
		...
		Page webpage = this.daoSupport.queryForPage(sql, pageNo, pageSize,member_id);
		return webpage;
	}
