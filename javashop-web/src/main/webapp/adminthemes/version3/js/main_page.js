		...
		tab = layui.tab({
			...
			element.on('tab(layout-tab)', function(data) {
				if(data.index===0){
					// 如果用户点击的是 首页 标签
					// 移除所有侧栏的active样式
					$('#side > ul a').removeClass('active');
					return;
				}
				// 切换至某个tab标题
				var tabTitle = $($('body > div.layui-layout > div.layui-tab > ul').find('li')[data.index]).find('cite').text();
				var done = false;
				$('#side > ul a').each(function(){
					if(done){
						return;
					}
					// 找到特定左侧栏目
					if($(this).find('cite').text() === tabTitle){
						// 先移除所有，再给当前增加样式
						$('#side > ul a').removeClass('active');
