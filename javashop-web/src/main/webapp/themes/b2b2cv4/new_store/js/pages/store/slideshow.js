...
$(function(){
    ...
    /* 商品图片ajax上传 */
    var url = SITE_URL + '/#?act=store&op=silde_image_upload';
    ...
    $('span[nctype="drop_image"]').unbind().click(function(){
        ...
        $.getJSON('#?act=store&op=dorp_img',{file_id:file_id,img_src:img_src}, function(data){
    ...
    (function(){
        if(lteIE8){
            var li = $('#goods_images').find('li'), liLen = li.length;
            li.removeClass('normal').css({
                float: 'left',
                width: (100/liLen-1)+'%'
            })
        };
