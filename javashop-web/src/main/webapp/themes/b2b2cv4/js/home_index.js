...
$(function () {
    setTimeout("takeCount()", 1000);
    //首页Tab标签卡滑门切换
    /*$(".tabnav > li > h3").on('mouseover', (function (e) {
        if (e.target == this) {
    ...
    //  修正逻辑使得楼层之间的tab显示不会互相干扰  _by: Andste 2016-10-15 20:36:54
    //  去掉每个楼层中第一个tab的隐藏class
    var index_middles = $('.index_middle');
    for(var i = 0, len = index_middles.length; i < len; i++){
        index_middles.eq(i).find('.tabs-panel').eq(0).removeClass('tabs-hide');
    }
    //  绑定鼠标移入事件
    $('.tabnav > li > h3').on('mouseover', function () {
        var _this = $(this), closestLi = _this.closest('li'), _index = closestLi.index();
        closestLi.addClass('tab_selected')
                 .siblings('li').removeClass('tab_selected');
        _this.closest('.index_middle')
             .find('.tabs-panel').eq(_index).removeClass('tabs-hide')
             .siblings('.tabs-panel').addClass('tabs-hide');
    })
