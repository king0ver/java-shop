...
$(function() {
    ...
    $("#quantity").keydown(function(e){
        ...
        var numObj = $("input[name='num']");//获取当前商品数量
...
function class_list(obj) {
	...
	if (span_class == 'goods_block') {
		$(".goods_store_category" + stc_id).show();
		$(obj).html('<em>-</em>');
		$(obj).attr('class', 'goods_none');
	} else {
		$(".goods_store_category" + stc_id).hide();
		$(obj).html('<em>+</em>');
		$(obj).attr('class', 'goods_block');
	}
}
