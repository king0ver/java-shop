...
function collect_store(store_id,obj,jsobj){
	var collect = $(obj).parent().find("#store_collect").text();
	$.getJSON( ctx + "/api/shop/member/is-login.do", function(data){
		if (data.result === 1) {
			if (data.data.state === 0) {
                location.href = ctx + '/store/login.html';
			}else{
				var url = ctx+"/api/b2b2c/store-collect/add-collect.do";
		        $.getJSON(url, {'store_id':store_id}, function(data){
		        	if(data.result === 1){
		        		$.message.success(data.message);
		        		$(obj).find("#store_collect").text(Number(collect)+1);
	...
	$.getJSON( ctx + "/api/shop/member/is-login.do", function(data){
		if (data.result === 1) {
			if (data.data.state === 0) {
		    	...
		    	$.Loading.show("正在收藏...");
				$.ajax({ 
					url:ctx+'/api/shop/collect/add-collect.do?goods_id='+fav_id,
					dataType:'json',
					cache: false,
					success:function(res){
						$.Loading.hide();
						res.result === 1 ? $.message.success(res.message) : $.message.error(res.message);
					}
				});
			}
		}else {
