...
(function($){
	...
	var DefLang ={
		...
		$.isMobile=function(val){var reg = /^0?(13[0-9]|15[0-9]|18[0-9]|14[0-9]|17[0-9])[0-9]{8}$/; return reg.test(val);};
		...
		var dt_mobile = function()
		{
		  this.check = function (val) {if(val=='') return true; return /^0?(13[0-9]|15[0-9]|18[0-9]|14[0-9]|17[0-9])[0-9]{8}$/.test(val);};
	...
	var Validator={
			...
			showNote:function(frm_ele,msg){
				var result = true;
				if(opts.showNote && typeof(opts.showNote  ) == 'function'){
					result = opts.showNote(frm_ele,msg);
				}
				if(result ) {
					...
					note_span.text(msg);
					if(frm_ele.is('.form-control')){
						frm_ele.parent().removeClass('has-error error').css({position: 'relative'});
						if(lteIE8){frm_ele.parent().css({marginBottom: ''})};
					};
				}
			},	
			...
			showOk:function(frm_ele,msg){
				...
				var result = true;
				if(opts.showOk && typeof(opts.showOk ) == 'function'){
					result = opts.showOk(frm_ele,msg);
				}
				...
				if(frm_ele.attr("dataType")!="" ||frm_ele.attr("isrequired")!=""  ||frm_ele.attr("fun")!=""){
					...
					if(frm_ele.is('.form-control')){
						frm_ele.parent().removeClass('has-error error').css({position: ''});
						if(lteIE8){frm_ele.parent().css({marginBottom: ''})};
					};
				}
			},
			...
			showError:function(frm_ele,msg){
				...
				if(result ) {
					...
					note_span.text(msg);
					if(frm_ele.is('.form-control')){
						frm_ele.parent().addClass('has-error error').css({position: 'relative'});
						if(lteIE8){frm_ele.parent().css({marginBottom: 0})};
					};
				}
			...
			getNoteSpan:function(frm_ele){
				...
				if(note_span && note_span.size()>0  ){
					...
					if(note_span && note_span.size()==0  ){
						frm_ele.parent().css({position: 'relative'});
						var left = frm_ele.position().left +'px', top = frm_ele.parent().height() + 5 + 'px';
						if(frm_ele.is('.form-control')){
							note_span = $("<span class=\"tip\" style='color:#a94442;font-size: 12px; top: "+ top +"; position: absolute; left: "+ left +";'></span>").insertAfter(frm_ele);
						}else {
							note_span = $("<span class=\"tip\" style='color:#a94442;font-size: 12px;'></span>").insertAfter(frm_ele);
						};
					}
				}
				return note_span;
		...
		inputs.each(function(){
			if(this){
				...
				if( el.attr("isrequired") || el.attr("dataType")|| el.attr("fun") || myvalidataor ){
					if ( !Validator.check( el ) ) { el.eq(0).focus(); result = false;}
				}
			}
		});
		return result;
	...
	$.fn.checkall=function(options){
		...
		if(checkAll(inputs)){
			...
			if(typeof($.message.error) == 'function'){
				$.message.error(DefLang.validate_fail);
			}else {
