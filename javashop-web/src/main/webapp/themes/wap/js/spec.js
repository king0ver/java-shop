...
var Spec={
	...
	loadProduct:function(goodsid){
		var self= this;
		$.ajax({
			url:ctx+"/api/shop/goods/product-list.do?goodsid="+goodsid,
			dataType:"json",
	...
	refresh:function(specLink){
		var self = this;
		var product_ar=[];
		$(".spec_item a.hovered").each(function(){
			var link = $(this);
...
,
	...
	addToCart:function(btn){
		...
		var options={
			url:ctx+"/api/store/store-cart/" + action + ".do?ajax=yes&store_id="+$("#storeid").val(),
			dataType:"json",
			async:false,
	...
	showCartCount:function(){
		$.ajax({
			url:ctx+"/api/store/store-cart/get-cart-data.do",   //获取购物车数据api
			dataType:"json",
			cache: false,             //清楚缓存，暂时测试，如果产生冲突，请优先考虑是否是这条语句。
			success:function(result){
				if(result.result==1){				
...
var PriceRefresh={
	refresh:function(pro){
		if(pro.length==1){
			$("#goods_price span").each(function(){
				var _self=$(this);
				var is_groupbuy=_self.attr("rel");
				if(typeof(is_groupbuy)=="undefined"||is_groupbuy==''||is_groupbuy==0){
					_self.text("￥"+pro[0].price );
				}
			})
			
			
			$(".goods_s").each(function(){
				var _self=$(this);
				var is_groupbuy=_self.attr("rel");
				if(typeof(is_groupbuy)=="undefined"||is_groupbuy==''||is_groupbuy==0){
					_self.text("库存："+pro[0].enable_store );
				}
			})
			...
			$("#goods_price span").text("￥"+minPrice+"-￥" +maxPrice);
		}
	}
};
function canBuy(){
...
var BtnTipRefresh = {
	refresh:function(pro){
		...
		if(pro.length==1){
			...
			var i=0;
			var tip='';
			$("#goodsform .spec_item em").each(function(){
				var em = $(this);
