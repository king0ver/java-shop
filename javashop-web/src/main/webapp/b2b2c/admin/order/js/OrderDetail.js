...
var OrderDetail = {
	...
	cancel	:undefined,
	isCod : false,
	init : function(orderid, orderStatus, payStatus, shipStatus, isCod,paymentid ,cancel) {
		// 初始化订单的状态
	...
	bindFlowEvent : function() {
		...
		$("#refund").bind("click",function() {Eop.Dialog.open("order_dialog");
			var disabled=  $("#refund").hasClass("layui-btn-disabled");
			if( !disabled ){
				$("#order_dialog .con").load(basePath+ "/payment/show-refund-dialog.do?orderId="+ self.orderid,
		...
		$("#returned").bind("click",function() {
			...
			if( !disabled ){
					newTab("申请退货","../shop/admin/sell-back/add-sellback.do?orderId="+ self.orderid);
			}
		});
		// 收货事件绑定
		$("#rog").unbind("click");
						...
						,content: $('#cancelorder') 
						  ...
						  ,yes: function(index, layero){
							  ...
							  if(!disabled){
								  ...
								  self.cancell(self.orderid,reason);
	...
	refund : function() {
		var self = this;
		var options = {
			url : basePath + "/payment/cancel_pay.do",
			type : "post",
			dataType : "json",
			success : function(responseText) {
				if (responseText.result == 1) {
	 ...
	 * 作废
	 */
	cancell : function(orderId, canel_reason) {
		var self = this;
		$.Loading.show("正在保存请稍候..");
		//savebtn.linkbutton("disable");
		$.ajax({
