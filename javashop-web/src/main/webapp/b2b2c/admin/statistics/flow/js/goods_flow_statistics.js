...
$(function(){
	...
	$("#search_statis").click(function(){
		...
		store_id=$('#store option:selected').val();
...
function initGoodsFlowStatistics(dateWhere) {
	// ajax配置
	var options = {
		url : ctx + "/b2b2c/admin/flowStatistics/get-goods-flow-statistics.do" ,
		data : {'start_date' : dateWhere[0], 'end_date' :dateWhere[1],'top_num':topNum,'store_id':store_id},
		type : "post",
		dataType:"json",
...
function initHistogram(conf,dateWhere){
    ...
    $.ajax({
	    ...
	    url : ctx + "/b2b2c/admin/flowStatistics/get-goods-flow-statistics.do",
	    data : {'start_date' : dateWhere[0], 'end_date' : dateWhere[1],'store_id':store_id},
