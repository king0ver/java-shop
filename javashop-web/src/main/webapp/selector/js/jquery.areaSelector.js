...
(function ($) {
    var areaHTML = "<div class='area-container'>\
            <style>\
				.area-container {\
					width: 692px;\
					height: 545px;\
...
\
    ...
    // ---------------------------------------全局变量---------------------------------------
    var areaData = [{}] // 所有,省市县数据.
    var areaDOM = $(areaHTML);  // DOM数据,后续操作全部基于此DOM
    var confirmCallback = null; // 全局回调变量
    ...
    var bindEventListener = function () {
        ...
        areaDOM.find('.model li > div .open-close').on('click', function (e) {
            ...
            var regionID = $(this).closest('li').attr('region-id');
            var depthOneID = $(this).closest('li.depth-one').attr('region-id');
            if ($(this).closest('li').hasClass('depth-one')) {
        ...
        $('.area-selector-container .foot .confirm').click(function () {
            $(this).closest('.area-container').css('display', 'none');
    ...
    // 左边div元素绑定点击事件.
    var bindClickListener = function (which) {
    ...
    // 请求并初次渲染数据.(仅渲染第一层,省级)
    var requestAndFirstRenderData = function () {
        $.getJSON(ctx + '/api/base/region/get.do?depth=3', function (response) {
            areaData = response;
    ...
    // where: 渲染出来的li,插入到哪里
    var renderDepthTwoData = function (regionID, where) {
        // 遍历各市,插入到HTML中
        var li;
        areaData.forEach(function (province) {
            // 找到用户点击的省份的对象
    ...
    // where: 渲染出来的li,插入到哪里
    var renderDepthThreeData = function (regionID, depthOneID, where) {
        // 遍历各市,插入到HTML中
        var li;
        areaData.forEach(function (province) {
            // 找到对应省份
    ...
    var toggleSelectedStyle = function (item) {
        // 切换自身的样式
        if (item.hasClass('selected')) {
            item.addClass('unselected').removeClass('selected');
            item.find('li').addClass('unselected').removeClass('selected');
            // 同时切换+/-符号的样式
            item.find('.open-close').removeClass('icon-grey');
        } else {
            item.addClass('selected').removeClass('unselected');
            item.find('li').addClass('selected').removeClass('unselected');
            // 同时切换+/-符号的样式
        ...
        if (item.parent().children('.selected').length === item.parent().children().length) {
            item.parent('ul').parent('li').removeClass('unselected').addClass('selected');
            // 同时切换+/-符号的样式
            item.parent('ul').prev().find('.open-close').addClass('icon-grey');

        } else {
            // 其他情况,父皆不选:
            // 如果所有兄弟元素包括自己全为不选.
            // 所有兄弟元素包括自己 有选又不选
            item.parent('ul').parent('li').removeClass('selected').addClass('unselected');
            // 同时切换+/-符号的样式
            item.parent('ul').prev().find('.open-close').removeClass('icon-grey');
        }
        // 如果用户点击的是第二层.return;
        // 后面的代码是用户点击第三层才执行的.
        ...
        var depthOne = item.closest('.depth-one').children('ul');
        if (depthOne.children('.selected').length === depthOne.children().length) {
            depthOne.parent('li').removeClass('unselected').addClass('selected');
            depthOne.prev().find('.open-close').addClass('icon-grey');
        } else {
            depthOne.parent('li').removeClass('selected').addClass('unselected');
            depthOne.prev().find('.open-close').removeClass('icon-grey');
        }
    };
    ...
    var addDomToRight = function () {
        ...
        areaDOM.find('.body .area-left li.depth-one').each(function () {
            ...
            // 右边,DOM
            var right = $(this).closest('.area-left').siblings('.area-right').children('ul');
            ...
            if ($(this).attr('data-all') === 'all') {
                ...
                // 取消图标白色样式
                $(this).find('.open-close').removeClass('icon-grey');
                $(this).find('div').off('click');

                var depthOneDOM = right.find('.depth-one[region-id=' + $(this).attr('region-id') + ']');
                ...
                if (depthOneDOM.length === 0) {
                    ...
                    $(this).find('li.depth-two').each(function () {
    ...
    var dealLiDOM = function (li, li_cloned) {
        // 源li
        // 遍历第二层li
        ...
        // 克隆li
        // 遍历第二层li
        li_cloned.children('ul').children('li').each(function () {
            // 不存在 data-all 属性
            if (!$(this).attr('data-all')) {
                // 克隆li删除
                $(this).remove();
            ...
            // 删除点击事件.(父,自身,子)
            $(this).closest('ul').prev().off('click');
            $(this).find('div').off('click'); // 包括自身和children
            // 移除class
            $(this).removeClass('selected');
            $(this).find('li').removeClass('selected');
    ...
    var appendOperation = function (rightDOM, theLiDOM) {
        // 如果当前li在右边的第一层DOM中未发现.
        // 根据邮编判断
        var depthOneDOM = rightDOM.find('.depth-one[region-id=' + theLiDOM.attr('region-id') + ']');
        if (depthOneDOM.length === 0) {
            // 直接添加
        ...
        theLiDOM.find('li.depth-two').each(function () {
            var depthTwoDOM = depthOneDOM.find('.depth-two[region-id=' + $(this).attr('region-id') + ']');
            // 如果当前第二层li在右边对应地方不存在
            ...
            // 如果第二层也在右边对应第一层里存在
            // 遍历并判断第三层
            $(this).find('li.depth-three').each(function () {
                var depthThreeDOM = depthTwoDOM.find('.depth-three[region-id=' + $(this).attr('region-id') + ']');
                // 如果当前第三层li在右边对应地方不存在
    ...
    // 在左边对应位置复现将被删除的地区
    var appearanceAtLeft = function (which) {
        // 左边DOM
        var leftDOM = which.closest('.area-right').siblings('.area-left').children('ul');
        // 用户点击的是第一层
        if (which.hasClass('depth-one')) {
            var leftParentOne = leftDOM.find('.depth-one[region-id=' + which.attr('region-id') + ']');
            // 该li在左边不存在
            if (leftParentOne.length === 0) {
                ...
                which.find('li.depth-two').each(function () {
                    var leftParentTwo = leftParentOne.find('.depth-two[region-id=' + $(this).attr('region-id') + ']');
                    // 当前第二层DOM在左边对应位置不存在
                    if (leftParentTwo.length === 0) {
                        ...
                        $(this).find('li.depth-three').each(function () {
                            // 第三层在左边不存在
                            if (leftParentTwo.find('.depth-three[region-id=' + $(this).attr('region-id') + ']').length === 0) {
                                // 绑定回点击事件
            ...
            // 用户点击的是第二层
            // 获取父,第一层DOM
            var parentDepthOne = which.closest('li.depth-one');
            // 在左边查找当前元素的第一层DOM
            var leftParentOne = leftDOM.find('.depth-one[region-id=' + parentDepthOne.attr('region-id') + ']');
            // 如果父第一层在左边不存在
            if (leftParentOne.length === 0) {
                // 先克隆下
                var cloneParent = parentDepthOne.clone(true);
                // 清空父第一层DOM的所有children,只添加当前第二层DOM
                cloneParent.children('ul').empty().prepend(which);
                // 绑定回点击事件
            ...
            var leftDepthTwo = leftParentOne.find('.depth-two[region-id=' + which.attr('region-id') + ']');
            // 当前第二层在左边不存在
            if (leftDepthTwo.length === 0) {
                var whichCloned = which.clone(true);
                // 绑定回点击事件
                ...
                which.find('li.depth-three').each(function () {
                    // 当前第三层在左边不存在
                    if (leftDepthTwo.find('.depth-three[region-id=' + $(this).attr('region-id') + ']').length === 0) {
                        // 绑定回点击事件
                        $(this).find('div').each(function () {
                            bindClickListener($(this));
                            // 转换样式
            ...
            // 用户点击的是第三层
            // 获取父,第二层DOM和第一层DOM
            var parentDepthTwo = which.closest('li.depth-two');
            var parentDepthOne = which.closest('li.depth-one');

            // 父第一层,父第二层DOM,在左侧对应的DOM
            var leftParentOne = leftDOM.find('.depth-one[region-id=' + parentDepthOne.attr('region-id') + ']');
            var leftParentTwo = leftDOM.find('.depth-two[region-id=' + parentDepthTwo.attr('region-id') + ']');

            // 如果父第一层在左边不存在
            if (leftParentOne.length === 0) {
                // 克隆下parent数据
                var cloneParentDepthTwo = parentDepthTwo.clone(true);
                var cloneParentDepthOne = parentDepthOne.clone(true);

                // 清空无用DOM,只有当前DOM子类
                cloneParentDepthTwo.children('ul').empty().prepend(which);
                cloneParentDepthOne.children('ul').empty().prepend(cloneParentDepthTwo);
                // 绑定回点击事件
    ...
    var cloneData = function (data) {
        var tempObj = JSON.parse(JSON.stringify(data));
    ...
    var getJSON = function () {
        ...
        areaDOM.find('.area-right li.depth-one').each(function () {
            ...
            var provinceID = Number($(this).attr('region-id'));
        ...
        areaDOM.find('.area-right li.depth-two').each(function () {
            var cityID = Number($(this).attr('region-id'));
            // 如果左边不存在对应市, 说明全选了市
            // 把所有县级数据添加进来
            if (areaDOM.find('.area-left li.depth-two[region-id=' + cityID + ']').length === 0) {
                var provinceID = Number($(this).closest('li.depth-one').attr('region-id'));
        ...
        areaDOM.find('.area-right li.depth-three').each(function () {
            var countyID = Number($(this).attr('region-id'));
            var cityID = Number($(this).closest('li.depth-two').attr('region-id'));
            var provinceID = Number($(this).closest('li.depth-one').attr('region-id'));
    ...
    var startDefault = function (sourceData) {
        if (!sourceData) {
            return;
        }
        // 划分源数据为三部分, level 1 , level 2 , level 3
        // 其中level 1,2 只留region_id
        var divideData = function (data) {
        ...
        var dividedData = divideData(sourceData);
        ...
        */
        var dealLevelThreeData = function (completeData, data, levelTwoData) {
            var levelTwoArray = levelTwoData.map(function (tempObj) {
         ...
         */
        var dealLevelTwoData = function (completeData, levelTwo, levelOne) {
        ...
        var setTime = setInterval(function () {
            if (areaDOM.find('.area-left li.depth-one[region-id=' + 1 + ']').length === 1) {
                clearInterval(setTime);
                // 遍历初始化省份
                dividedData[0].forEach(function (provinceID) {
                    areaDOM.find('.area-left li.depth-one[region-id=' + provinceID + '] > div').click();
                });
                // 遍历初始化市
                dividedData[1].forEach(function (city) {
                    // 找到特定省/市
                    var provinceDOM = areaDOM.find('.area-left li.depth-one[region-id=' + city.p_regions_id + ']');
                    provinceDOM.children('div').find('.open-close').click();

                    var cityDOM = provinceDOM.find('li.depth-two[region-id=' + city.region_id + ']');
                    cityDOM.children('div').click();
                });
                // 遍历初始化县
                dividedData[2].forEach(function (county) {
                    // 找到特定省/市/县
                    var provinceDOM = areaDOM.find('.area-left li.depth-one[region-id=' + county.p_p_regions_id + ']');
                    // 如果不存在市级数据,说明还没有点击过 +
                    if (provinceDOM.find('li.depth-two').length === 0) {
                        provinceDOM.children('div').find('.open-close').click();
                    }
                    var cityDOM = provinceDOM.find('li.depth-two[region-id=' + county.p_regions_id + ']');
                    // 如果不存在县级数据,说明还没有点击过 +
                    if (cityDOM.find('li.depth-three').length === 0) {
                        cityDOM.children('div').find('.open-close').click();
                    }
                    var countyDOM = provinceDOM.find('li.depth-three[region-id=' + county.region_id + ']');
