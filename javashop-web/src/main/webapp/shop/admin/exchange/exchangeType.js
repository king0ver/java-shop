var Prop={
	...
	initinputHtml:function(){
		...
		$.ajax({
			 type: "POST",
			 url: ctx+"${ctx}/shop/admin/exchange-type/prop_input_item.html?ajax=yes",
			 dataType:'html',
...
var Param={
		...
		initInputHtml:function(){
			...
			$.ajax({
				 type: "get",
				 url: ctx+"/shop/admin/exchange-type/param-item.do",
				 dataType:'html',
				 success: function(html){ 
					self.inputHtml=html;
				 },
...
var GoodsType=$.extend({
	...
	doDelete:function(){
		...
		this.deletePost(ctx+"${ctx}/shop/admin/exchange-type/delete.do");
	...
	doRevert:function(){
		...
		this.deletePost(ctx+"${ctx}/shop/admin/exchange-type/revert.do","选择的类型已被成功还原至类型列表中");		
	},
	intChkNameEvent:function(){
		$(".submitlist .submitBtn").click(function(){
			...
			var name = $("#name").val();
			$("form").ajaxSubmit({
				url:'${ctx}/shop/admin/exchange-type/checkname.do?ajax=yes',
				type:'POST',
				dataType:'json',
