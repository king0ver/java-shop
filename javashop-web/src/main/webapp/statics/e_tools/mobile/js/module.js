...
Module.prototype.scrollToTopControl = (function () {
    ...
    function _init(opts) {
        ...
        if (typeof options.clickToTop !== 'function') {
            $(document).on('tap', '#control-to-top', function () {
                $('body,html').animate({ scrollTop: 0 }, 300, _hide);
                return false
            })
        } else {
			$(document).on('tap', '#control-to-top', function () {
    ...
    function _show() {
        if($('#control-to-top').is('.open')) return;
        toTop.addClass('open');
        setTimeout(function () {
            toTop.addClass('show')
        }, 20)
    ...
    function _hide() {
		if(!$('#control-to-top').is('.show')) return;
    ...
    function rollingListening() {
        $(window).scroll(function () {
			$(window).scrollTop() > 150 ?_show() : _hide();
        })
    }
