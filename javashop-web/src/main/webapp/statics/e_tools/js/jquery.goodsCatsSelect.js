...
;(function ($, window, document, undefined) {
    var goodsCatsSelect = function (ele, opts) {
        ...
        this.default = {
            'host'    : ctx,
            'api'     : '/api/store-statistics/cat-list-json.do'
        };
    ...
    goodsCatsSelect.prototype = {
        ...
        countCss: function () {
            ...
            _position && _position === 'top'
                ? _offsetTop -= (402 + _offsetHeight)
                : _offsetTop += _offsetHeight;
            return {
                left   : _offsetLeft,
                top    : _offsetTop,
        ...
        processingCatList: function (_res) {
            ...
            var count = this.conutLevel(res[0]['category_path']);
            if(count === 2){
        ...
        appendContent: function (count, _val) {
            ...
            for(var i = 0; i < res.length; i++){
                _item += '<div class="__cat_item" data-id="'+ res[i]['id'] +'" data-path="'+ res[i]['category_path'] +'">\
                        <h4>'+ res[i]['text'] +'</h4>\
                        <div class="__cat_item_btns">\
                            <i class="__icon __edit" data-id="'+ res[i]['id'] +'" data-text="'+ res[i]['text'] + '" data-leave="'+ (count - 1) +'"></i>\
