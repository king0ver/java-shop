...
public class ActReserveServiceImpl implements IActReserveService  {
	...
	public void cancelReserve(NanShanActReserve NanShanActReserve) {
		this.daoSupport.execute("update es_nanshan_act_reserve set is_del=1 where article_id="+NanShanActReserve.getActivity_id()+" and member_id="+NanShanActReserve.getMember_id());
	...
	public Page<ActReserveVo> queryReserveListById(Integer userId, int pageNo,
			int pageSize) {
		String sql ="select a.id articleId, a.title activityName, t.is_del isDel, t.activity_time  activityTime from es_nanshan_article a ,es_nanshan_act_reserve t where a.id=t.article_id and t.member_id="+userId+" order by t.activity_time";
