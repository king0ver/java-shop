...
/**
 * 订单发货发送消息
 * 订单发货发送消息包含：邮件、短信、站内信
 * @author kanon
 * @version v1.0
 * @since v6.4.0 2017年9月8日
 */
@Component
public class OrderShipMessageSendConsumer implements IOrderStatusChangeEvent{
	...
	public void orderChange(OrderStatusChangeMessage orderMessage) {
	 ...
	 /**
     * 发送会员消息 获取会员订单发货消息模板 判断是否开启站内信、短信、邮件 发送站内信、短信、邮件内容
     *
     * @param orderPo 店铺订单
     */
    private void memberMessageSend(OrderPo orderPo) {
        // 记录会员订单取消信息（会员中心查看）
        MessageTemplate messageTemplate = messageManager.getMessageTemplate(MessageCodeEnum.MEMBERORDERSSEND.name());
        if(messageTemplate != null) {
            ...
            if (messageTemplate.getEmail_state() == MessageStatusEnum.OPEN.getIndex()) {
                sendEmail(replaceEmailContent(messageTemplate.getEmail_content(), orderPo), orderPo, memberManager.get(orderPo.getMember_id()).getEmail(),
    ...
    private void storeMessageSend(OrderPo orderPo) {
        // 记录店铺订单取消信息（商家中心查看）
        MessageTemplate messageTemplate = messageManager.getMessageTemplate(MessageCodeEnum.MEMBERORDERSSEND.toString());
        if(messageTemplate != null) {
        	// 判断站内信是否开启
            if (messageTemplate.getNotice_state() == MessageStatusEnum.OPEN.getIndex()) {
            ...
            if (messageTemplate.getEmail_state() == MessageStatusEnum.OPEN.getIndex()) {
                ...
                sendEmail(replaceEmailContent(messageTemplate.getEmail_content(), orderPo), orderPo, shopManager.getShopDetail(orderPo.getSeller_id()).getCompant_email(),
                        messageTemplate.getEmail_title());
            }
        }
    }
    ...
    private String replaceContent(String content, OrderPo orderPo) {
        ...
        valuesMap.put("shipSn", orderPo.getShip_no());
    ...
    private String replaceEmailContent(String emailContent, OrderPo orderPo) {
        ...
        try {
            ...
            root.put("sendTime", DateUtil.toString(DateUtil.getDateline(), "yyyy-MM-dd HH:mm:ss"));
            root.put("siteName", EopSite.getInstance().getSitename());
            root.put("shipSn", orderPo.getShip_no());
     ...
     */
    private void sendEmail(String emailContent, OrderPo orderPo, String email, String emailTitle) {
