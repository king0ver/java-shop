 ...
 */
@Service
public class MemberRegisterConsumer implements IMemberRegisterEvent {
	...
	public void memberRegister(MemberRegistVo vo) {
		/* 获取会员信息 */
		Member member = memberManager.get(vo.getMember_id());
		/* 发送会员消息 */
	...
	private void memberMessageSend(Member member){
		// 记录会员登陆成功信息（会员中心查看）
		MessageTemplate messageTemplate = messageManager.getMessageTemplate(MessageCodeEnum.MEMBERREGISTESUCCESS.name());
