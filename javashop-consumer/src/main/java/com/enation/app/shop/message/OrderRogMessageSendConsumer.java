...
/**
 * 订单收货发送消息
 * 订单收货发送消息包含：邮件、短信、站内信
 * @author kanon
 * @version v1.0
 * @since v6.4.0 2017年9月8日
 */
@Component
public class OrderRogMessageSendConsumer implements IOrderStatusChangeEvent{
	...
	/**
     * 发送会员消息 获取订单收货消息模板 判断是否开启站内信、短信、邮件 发送站内信、短信、邮件内容
     *
     * @param orderPo 店铺订单
     */
    private void memberMessageSend(OrderPo orderPo) {
        // 记录会员订单取消信息（会员中心查看）
        MessageTemplate messageTemplate = messageManager.getMessageTemplate(MessageCodeEnum.MEMBERORDERSRECEIVE.toString());
    ...
    /**
     * 发送店铺消息 获取订单收货消息模板 判断是否开启站内信、短信、邮件 发送站内信、短信、邮件内容
     *
     * @param orderPo 店铺订单
     */
    private void storeMessageSend(OrderPo orderPo) {
        // 记录店铺订单取消信息（商家中心查看）
        MessageTemplate messageTemplate = messageManager.getMessageTemplate(MessageCodeEnum.STOREORDERSRECEIVE.toString());
        if(messageTemplate != null) {
        	 // 判断站内信是否开启
            if (messageTemplate.getNotice_state() == MessageStatusEnum.OPEN.getIndex()) {
    ...
    private String replaceEmailContent(String emailContent, OrderPo orderPo) {
        ...
        try {
            ...
            root.put("finishTime", DateUtil.toString(orderPo.getCreate_time(), "yyyy-MM-dd HH:mm:ss"));
